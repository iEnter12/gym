...
class Booking(models.Model):
    ...
    confirm_time = models.DateTimeField(blank=True, null=True, verbose_name='确定时间')
    ...
    def can_checkin(self):
        """是否可以签到"""
        # 已确定状态且未签到过即可签到（移除时间限制）
        return (self.status == 1 and self.checkin_time is None)
    ...
    def can_confirm(self):
        ...
        return (self.status == 0 and 
                timezone.now() <= self.create_time + timedelta(minutes=15))
